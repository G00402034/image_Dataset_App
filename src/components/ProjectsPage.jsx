  ...
  const confirmDeleteProject = async () => {
    ...
    try {
      if (onProjectDelete) {
        ...
        await api.projects.delete(projectToDelete._id || projectToDelete.id);
        await fetchProjects();
      }
    } catch (e) {
      console.error(e);
          ...
          <div style={styles.projectsGrid}>
            {projects.map(project => (
              <div 
                ...
                style={{
                  ...styles.projectCard,
                  ...(currentProject?.id === project.id || currentProject?._id === project._id ? styles.activeProject : {})
                }}
              >
                ...
                <div style={styles.projectStats}>
                  <div style={styles.stat}>
                    ...
                    <span style={styles.statValue}>{project.imageCount || project.images?.length || 0}</span>
                  </div>
                  <div style={styles.stat}>
          ...
          <div style={styles.modal}>
            ...
            <div style={styles.modalContent}>
              <p style={styles.warningText}>
                ...
                This action cannot be undone and will permanently remove all images.
              </p>
              <div style={styles.modalActions}>
                <button 
                  onClick={() => setShowDeleteModal(false)}
...
const styles = {
  ...
  header: { display: 'flex', alignItems: 'center', gap: '20px', marginBottom: '30px', paddingBottom: '20px', borderBottom: '1px solid #e9ecef' },
  ...
  projectStats: { marginBottom: '16px' },
  stat: { display: 'flex', justifyContent: 'space-between', marginBottom: '8px' },
  statLabel: { color: '#6c757d', fontSize: '14px' },
  ...
  emptyTitle: { margin: '0 0 12px 0', color: '#6c757d', fontSize: '20px', fontWeight: '500' },
  emptyText: { margin: '0 0 24px 0', color: '#6c757d', fontSize: '16px' },
  modalOverlay: { position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, backgroundColor: 'rgba(0, 0, 0, 0.5)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 1000 },
  modal: { backgroundColor: '#ffffff', borderRadius: '12px', maxWidth: '500px', width: '90%', maxHeight: '80vh', overflow: 'hidden', boxShadow: '0 10px 30px rgba(0, 0, 0, 0.3)' },
  modalHeader: { display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '20px', borderBottom: '1px solid #e9ecef' },
  closeButton: { background: 'none', border: 'none', fontSize: '20px', color: '#6c757d', cursor: 'pointer' },
  modalContent: { padding: '20px' },
  inputGroup: { marginBottom: '20px' },
  label: { display: 'block', marginBottom: '8px', color: '#495057', fontSize: '14px', fontWeight: '500' },
  input: { width: '100%', padding: '12px', border: '1px solid #dee2e6', borderRadius: '6px', fontSize: '14px' },
  modalActions: { display: 'flex', gap: '12px', justifyContent: 'flex-end' },
  cancelButton: { padding: '10px 20px', backgroundColor: '#6c757d', color: '#ffffff', border: 'none', borderRadius: '6px', fontSize: '14px', cursor: 'pointer' },
  confirmButton: { padding: '10px 20px', backgroundColor: '#007bff', color: '#ffffff', border: 'none', borderRadius: '6px', fontSize: '14px', cursor: 'pointer' },
  deleteConfirmButton: { padding: '10px 20px', backgroundColor: '#dc3545', color: '#ffffff', border: 'none', borderRadius: '6px', fontSize: '14px', cursor: 'pointer' },
  disabledButton: { backgroundColor: '#6c757d', cursor: 'not-allowed' },
  warningText: { color: '#dc3545', fontSize: '14px', lineHeight: '1.5', marginBottom: '20px' }
};
